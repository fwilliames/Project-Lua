    ...
    function actions.build()
        ...
        local fireBall = {
            ...
            execute = function (playerData, creatureData)
                ...
                if sucess then
                    print(
                        string.format("%s acertou a %s e deu %d pontos de dano de fogo!",playerData.name,creatureData.name,damage)
                    )
        ...
        local manaRegenPotion = {
            description = "toma uma poção de regeneração de mana",
            ...
            execute = function (playerData, creatureData)
                --Retira um pote do inventario do Player
                playerData.manaPotion = playerData.manaPotion - 1

                --Recuperando vida
                local regenPoints = 5
                playerData.mana = math.min(playerData.maxMana,playerData.mana + regenPoints)
                print("Voce usou um pote de regeneracao e recuperou alguns pontos de mana")

            end
        }
        actions.list[#actions.list + 1] = fireBall
        actions.list[#actions.list + 1] = manaRegenPotion
