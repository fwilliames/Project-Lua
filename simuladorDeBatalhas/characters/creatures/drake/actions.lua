...
local actions = {}
    ...
    function actions.build()
        ...
        local fireAttack = {
            description = "Cospe fogo no oponente",
            requirement = nil,
            execute = function (playerData, creatureData)
                ...
                if sucess then
                    print(
                        string.format("%s queimou  %s e deu %d pontos de dano!",creatureData.name,playerData.name,damage)
                    )
        ...
        local wingAttack = {
            description = "Empurra o inimigo com a força do vento",
            requirement = nil,
            execute = function (playerData, creatureData)
                --Calculo de dano
                local rawDamage = creatureData.attack - math.random() * playerData.defense
                ...
                print(
                    string.format("%s empurrou com o vento e deu %d pontos de dano!",creatureData.name,damage)
                )
                playerData.health = playerData.health - damage
                local healthRate = math.floor((playerData.health / playerData.maxHealth) * 10)
        ...
        actions.list[#actions.list + 1] = wingAttack
